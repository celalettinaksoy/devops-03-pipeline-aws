    ...
    stages {
        ...
        */
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-35-169-94-227.compute-1.amazonaws.com:8080/job/devops-03-pipeline-aws-gitops/buildWithParameters?token=GITOPS_TRIGGER_START'"
                }
            }
        }
    }
